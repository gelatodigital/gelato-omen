...
import {GelatoOracleAggregator} from "../../external/OracleAggregator.sol";
...
contract ActionWithdrawLiquidity is GelatoActionsStandard {
    ...
    GelatoOracleAggregator public immutable oracleAggregator;

    constructor(IGelatoSysAdmin _gelatoCore, address _provider, IERC20 _WETH, IUniswapV2Router02 _uniRouter, GelatoOracleAggregator _oracleAggregator) public {
        ...
        virtual
    {
        ...
        if (address(WETH) == _collateralToken) collateralTokenFee = ethToBeRefunded;
            ...
            try oracleAggregator.getExpectedReturnAmount(ethToBeRefunded, ETH, _collateralToken) returns (uint256 returnAmount, uint256) {
                collateralTokenFee = returnAmount.div(1 ether);
            } catch {
                collateralTokenFee = getUniswapRate(address(WETH), ethToBeRefunded, _collateralToken);
            }
        }
